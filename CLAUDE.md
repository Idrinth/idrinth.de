...
The generated `output/` and `public/` files are committed to git. The build detects template changes via MD5 hashing (`.template-hash`) and regenerates only when needed (or when post content changes).
